  ...
  describe('Unit Tests', function() {
    describe('#mergeData', function() {
      ...
      it('emits a loaded event with the new data', function() {
        ...
        this.brain.mergeData({});
        expect(this.brain.emit).to.have.been
          .calledWith('loaded', this.brain.data);
      });
    });
    ...
    describe('#save', function() {
      it('emits a save event', function() {
        ...
        this.brain.save();
        expect(this.brain.emit).to.have.been
          .calledWith('save', this.brain.data);
      });
    });
    ...
    describe('#usersForRawFuzzyName', function() {
      it('does a case-insensitive match', function() {
        expect(this.brain.usersForRawFuzzyName('guy'))
          .to.have.members([this.user1, this.user2]);
      });
      it('returns all matching users (prefix match) when there is not an ' +
        'exact match (case-insensitive)', function() {
        expect(this.brain.usersForRawFuzzyName('Guy'))
          .to.have.members([this.user1, this.user2]);
      });
      it('returns all matching users (prefix match) when there is an ' +
        'exact match (case-insensitive)', function() {
        // Matched case
        expect(this.brain.usersForRawFuzzyName('Guy One'))
    ...
    describe('#usersForFuzzyName', function() {
      it('does a case-insensitive match', function() {
        expect(this.brain.usersForFuzzyName('guy'))
          .to.have.members([this.user1, this.user2]);
      });
      it('returns all matching users (prefix match) when there is not an ' +
      ...
      it('returns just the user when there is an exact ' +
        'match (case-insensitive)', function() {
        // Matched case
        expect(this.brain.usersForFuzzyName('Guy One'))
